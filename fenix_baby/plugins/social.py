...
async def couple_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == ChatType.PRIVATE: return await update.effective_message.reply_text("❌ Group Only!", parse_mode=ParseMode.HTML)
    ...
    if target:
        ...
        if int(user2['user_id']) == int(OWNER_ID):
             return await update.effective_message.reply_text("👑 <b>Blasphemy!</b> You cannot match with the Creator. His heart belongs to the code.", parse_mode=ParseMode.HTML)
    else:
        try:
            # Random Match in Group
            pipeline = [{"$match": {"seen_groups": chat.id, "user_id": {"$ne": user.id}}}, {"$sample": {"size": 1}}]
            results = list(users_collection.aggregate(pipeline))
            if not results: return await update.effective_message.reply_text("😔 No one else found here.", parse_mode=ParseMode.HTML)
            user2 = results[0]
        except:
             return await update.effective_message.reply_text("⚠️ DB Error", parse_mode=ParseMode.HTML)
...
async def propose(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sender.get("partner_id"): return await update.effective_message.reply_text("❌ Married!", parse_mode=ParseMode.HTML)
    
    target_arg = context.args[0] if context.args else None
    target, error = await resolve_target(update, context, specific_arg=target_arg)
    
    if not target: return await update.effective_message.reply_text(error or "⚠️ <b>Usage:</b> <code>/propose @user</code>", parse_mode=ParseMode.HTML)
    
    # Owner Protection
    from fenix_baby.config import OWNER_ID
    if int(target['user_id']) == int(OWNER_ID):
        return await update.effective_message.reply_text("👑 <b>Impossible!</b> The Creator is already married to his work and the universe. You cannot propose to a God.", parse_mode=ParseMode.HTML)
    
    if target['user_id'] == sender['user_id']: return await update.effective_message.reply_text("🥲 No.", parse_mode=ParseMode.HTML)
    if target.get('partner_id'): return await update.effective_message.reply_text("💔 Taken.", parse_mode=ParseMode.HTML)
...
async def marry_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target and int(target['user_id']) == int(OWNER_ID):
        return await update.effective_message.reply_text("👑 <b>The Creator is beyond status.</b> His only partnership is with his code.", parse_mode=ParseMode.HTML)
...
async def divorce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user.get("partner_id"): return await update.effective_message.reply_text("🤷‍♂️ Single.", parse_mode=ParseMode.HTML)
    if user['balance'] < DIVORCE_COST: return await update.effective_message.reply_text(f"❌ Cost: {format_money(DIVORCE_COST)}", parse_mode=ParseMode.HTML)
    ...
    await update.effective_message.reply_text(f"💔 <b>Divorced!</b> Paid {format_money(DIVORCE_COST)}.", parse_mode=ParseMode.HTML)
...
async def proposal_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "marry_y":
        ...
        await query.message.edit_text("❌ <b>Rejected!</b> Better luck next time. 💔", parse_mode=ParseMode.HTML)
